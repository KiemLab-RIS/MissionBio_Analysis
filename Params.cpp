...
std::pair<std::string,int> Params::identifySequence(std::string seq) {
	...
	// allow 1 error in primer
	    ...
	    // allow 19 errors in fwd seq comp with amplicon
            if (a > 19) {
                return std::make_pair(p->ampID,-2);
//                int start = 0;
//                int l   = 100;
...
//std::map<std::string, std::vector<EditEntry>>
EditMap read_gene_csv(const std::string& filename)
{
    //std::map<std::string, std::vector<EditEntry>> result;
    EditMap result;
	...
	if (line[0] == '#') continue;   // <-- skip comment lines
        ...
        std::string gene, location_str, base_str;

        std::getline(ss, gene, ',');
        std::getline(ss, location_str, ',');
        std::getline(ss, base_str, ',');

	std::cout << gene << " " << location_str << " " << base_str << "\n";

        EditEntry e;
        e.location = std::stoi(location_str);
        e.base = base_str.at(0);

        result[gene].push_back(e);
    }
